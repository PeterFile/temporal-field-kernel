...
- `tfk-protocol`: wire contracts for observations, continuations, lenses, preflight signals, forecasts, commits, and envelopes.
- `tfk-core`: deterministic continuation, temporal lens, preflight, and scoring logic.
- `tfk-store`: append-only raw archive plus SQLite projections for historical influence, continuations, and lens grounding.
- `tfk-rules`: embedded Datalog-like fixed-point rules for deriving influence and consequence signals.
- `tfk-api`: local daemon API surface for observing, querying lenses, checking preflight risk, and recording continuations.
